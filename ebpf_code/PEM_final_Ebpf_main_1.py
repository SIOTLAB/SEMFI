...
#include <bcc/proto.h>
#include "/home/ans2/Desktop/linux51080/include/net/mac80211.h"
#include "/home/ans2/Desktop/linux51080/net/mac80211/ieee80211_i.h"
#include <uapi/linux/ptrace.h>
#include <linux/dma-mapping.h>
#include "/home/ans2/Desktop/linux51080/drivers/net/wireless/ath/ath9k/ath9k.h"
#include </home/ans2/Desktop/linux51080/include/linux/ieee80211.h>
#include </home/ans2/Desktop/linux51080/include/uapi/linux/time.h>
  ...
  struct list_head *bf_head)
{
    ...
    */
    ...
    for (int i = 0; i < num_devices; i++) {
        if( (match_addr[i][0] == addr1[0] && match_addr[i][1] == addr1[1] && match_addr[i][2] == addr1[2])) {
            bpf_trace_printk("%d", i);
            int index = i + 1;
            bpf_trace_printk("STARTING_MATCHING_DEVICE_%d", index); 
            ...
            bpf_trace_printk("ENDING_MATCHING_DEVICE_%d", index); 
            break;
        }
    }
    
    /*if( (match_addr1[0] ==  addr1[0] && match_addr1[1] ==  addr1[1] && match_addr1[2] ==  addr1[2]))
    {
        bpf_trace_printk("STARTING_MATCHING_DEVICE_1_WITH_MAC_ADDR::%x:%x:%x", addr1[0], addr1[1], addr1[2]); 
        bpf_trace_printk("[2021][ath_tx_process_bufffer][mac80211_qnum] ... %d\\n", txq->mac80211_qnum);
        bpf_trace_printk("[2021][ath_tx_process_bufffer][axq_qnum] ... %d\\n", txq->axq_qnum);
        ...
        bpf_trace_printk("[2021][ath_tx_process_buffer]::7::peerd[1] %x:%x:%x\\n", addr1[0], addr1[1], addr1[2]);
        bpf_trace_printk("[2021][ath_tx_process_buffer]::7::peerd[2] %x:%x:%x\\n", addr1[3], addr1[4], addr1[5]);
        bpf_trace_printk("ENDING_MATCHING_DEVICE_1_WITH_MAC_ADDR");
    }
    else if( (match_addr2[0] ==  addr1[0] && match_addr2[1] ==  addr1[1] && match_addr2[2] ==  addr1[2]))
    {
        bpf_trace_printk("STARTING_MATCHING_DEVICE_2_WITH_MAC_ADDR::%x:%x:%x", addr1[0], addr1[1], addr1[2]); 
        bpf_trace_printk("[2021][ath_tx_process_bufffer][mac80211_qnum] ... %d\\n", txq->mac80211_qnum);
        bpf_trace_printk("[2021][ath_tx_process_bufffer][axq_qnum] ... %d\\n", txq->axq_qnum);
        ...
        bpf_trace_printk("[2021][ath_tx_process_buffer]::7::peerd[1] %x:%x:%x\\n", addr1[0], addr1[1], addr1[2]);
        bpf_trace_printk("[2021][ath_tx_process_buffer]::7::peerd[2] %x:%x:%x\\n", addr1[3], addr1[4], addr1[5]);
        bpf_trace_printk("ENDING_MATCHING_DEVICE_2_WITH_MAC_ADDR");
    }*/
 ...
 struct sk_buff *skb, struct napi_struct *napi)
{
    ...
    if( (match_addr1[0] ==  addr2[0] && match_addr1[1] ==  addr2[1] && match_addr1[2] ==  addr2[2]))
    {
        bpf_trace_printk("STARTING_MATCHING_DEVICE_1_WITH_MAC_ADDR::%x:%x:%x", addr1[0], addr1[1], addr1[2]);
        bpf_trace_printk("[2021][ath10k_process_rx]RECEIVED a [data] packet::2::::7::peerd[1] %x:%x:%x\\n", addr2[0], addr2[1], addr2[2]);
        bpf_trace_printk("[2021][ath10k_process_rx]RECEIVED a [data] packet::2::::7::peerd[2] %x:%x:%x\\n", addr2[3], addr2[4], addr2[5]);
    ...
    if( (match_addr2[0] ==  addr1[0] && match_addr2[1] ==  addr1[1] && match_addr2[2] ==  addr1[2]))
    {
        bpf_trace_printk("STARTING_MATCHING_DEVICE_2_WITH_MAC_ADDR::%x:%x:%x", addr1[0], addr1[1], addr1[2]);
        bpf_trace_printk("[2021][ath10k_process_rx]RECEIVED a [data] packet::2::::7::peerd[1] %x:%x:%x\\n", addr2[0], addr2[1], addr2[2]);
        bpf_trace_printk("[2021][ath10k_process_rx]RECEIVED a [data] packet::2::::7::peerd[2] %x:%x:%x\\n", addr2[3], addr2[4], addr2[5]);
    ...
    if( (match_addr2[0] ==  addr2[0] && match_addr2[1] ==  addr2[1] && match_addr2[2] ==  addr2[2]))
    {
        bpf_trace_printk("STARTING_MATCHING_DEVICE_2_WITH_MAC_ADDR::%x:%x:%x", addr2[0], addr2[1], addr2[2]);
        bpf_trace_printk("[2021][ath10k_process_rx]RECEIVED a [data] packet::2::::7::peerd[1] %x:%x:%x\\n", addr2[0], addr2[1], addr2[2]);
        bpf_trace_printk("[2021][ath10k_process_rx]RECEIVED a [data] packet::2::::7::peerd[2] %x:%x:%x\\n", addr2[3], addr2[4], addr2[5]);
 ...
 struct sk_buff *skb, struct napi_struct *napi)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if( (match_addr[i][0] ==  addr2[0] && match_addr[i][1] ==  addr2[1] && match_addr[i][2] ==  addr2[2])) {
            int index = i + 1;
            bpf_trace_printk("STARTING_MATCHING_DEVICE_%d", index); 
            bpf_trace_printk("[2021][ath10k_process_rx]RECEIVED a [data] packet::2::::7::peerd[1] %x:%x:%x\\n", addr2[0], addr2[1], addr2[2]);
            bpf_trace_printk("[2021][ath10k_process_rx]RECEIVED a [data] packet::2::::7::peerd[2] %x:%x:%x\\n", addr2[3], addr2[4], addr2[5]);
