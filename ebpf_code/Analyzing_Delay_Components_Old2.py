...
def periodic_energy_measurment (start_time, end_time, Lines):
...
for i in range(0, len(Lines)):
    ...
    if substring in line:
        ...
        substring = "LI"
        if i+3 < len(Lines) and substring in Lines[i+1] and "AID" in Lines[i+3]:
...
for i in range (len(index_list)):
    ...
    if MODE == 0:
      ...
      periodic_energy_measurment(exp_start_time, exp_end_time, Lines)
    if MODE == 1:
      for t in range(0, int(exp_end_time) - int(exp_start_time)):
        print("***********", exp_start_time+t, "--t-->", exp_start_time + t+1, "***********")
        periodic_energy_measurment(exp_start_time+t, exp_start_time + t+1, Lines)
    ...
    if MODE == 2:
      for t in np.arange(0, int(exp_end_time) - int(exp_start_time), INTERVAL):
        ...
        if ((exp_start_time + t+INTERVAL) - (exp_start_time+t)) < 1:
          ...
          periodic_energy_measurment(exp_start_time+t, exp_start_time + t+INTERVAL, Lines)
          # print("lines_processed--",lines_processed)
    ...
    for I in DEVICES.instances:
      # if I.MACADDR == MAC_match_list[0]:
        print(len(I.tx_timestamp))
        plt.plot(start_time_list, I.list_percentwakeperinterval)
        plt.scatter(I.tx_timestamp, I.tx_notifier_waste, marker='v',s=s)
        plt.scatter(I.beacon_wakeup_ts, I.beacon_wakeup_ts_notifier, marker='x',s=s)
        plt.scatter(I.rx_data_timestamp, I.rx_data_notifier, marker='^',s=s)
        plt.scatter(I.rx_PS_timestamp, I.rx_PS_notifier, marker='>',s=s)
        plt.scatter(I.beacon_tims_ts, I.beacon_tims_notifier, marker='*',s=s)
        plt.scatter(I.packet_arrival_ts, I.packet_arrival_notifier, marker='d',s=s)
